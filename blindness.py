...
y = True  # Always True since login is skipped

# Function to simulate file upload and model prediction
def OpenFile():
    username = box1.get()
    if y:
        try:
            ...
            value, classes = main(a)
            messagebox.showinfo("Your Report", f"Predicted Label: {value}\nPredicted Class: {classes}")

            # Display image and prediction
            image = Image.open(a)
            file = image.convert('RGB')
            plt.imshow(np.array(file))
            plt.title(f'Predicted Label: {value}, Class: {classes}')
            plt.axis('off')
            plt.show()
            ...
            print("Error during file selection or prediction:", error)
            messagebox.showerror("Error", "File not selected or error in processing. Please try again.")
    else:
        messagebox.showinfo("Login Required", "You need to log in first.")
...
# Disable login/signup buttons, just use image prediction
button2 = Button(root, text="Upload Image", command=OpenFile)
button2.grid(padx=10, pady=20, row=2, column=3)
